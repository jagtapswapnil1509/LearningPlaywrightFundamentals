...
- Name: 07_WebTables\254_Filter_Pagelocator.spec.ts >> verify Element by filter
- Location: tests\07_WebTables\254_Filter_Pagelocator.spec.ts:3:1
...
Call log:
  - waiting for locator('a.list-group-item').filter({ hasText: '/^Forgotten/' })
...
```ts
  ...
  6  | 
  7  |     //const forgotpasswordlink =await page.locator("a.list-group-item").filter({ hasText :'Forgotten Password'});
  8  |     //alternate way for filter
  9  |     const forgotpasswordlink =await page.locator("a.list-group-item").filter({ hasText :'/^Forgotten/'});
> 10 |     await forgotpasswordlink.click();
     |                              ^ Error: locator.click: Test timeout of 30000ms exceeded.
