...
- Name: Practice\practice2.spec.ts >> verify student login and validate the email id and password in URL
- Location: tests\Practice\practice2.spec.ts:4:1
...
Expected substring: "jagtap.swapnil@gmail.com"
Received string:    "https://app.thetestingacademy.com/playwright/multiple_element_filter?email=jagtap.swapnil%40gmail.com&password=12345&remember=yes#login-success"
```
...
```ts
  ...
  3  | 
  4  | test('verify student login and validate the email id and password in URL', async ({ page }) => {
  5  | 
  6  |     await page.goto("https://app.thetestingacademy.com/playwright/multiple_element_filter");
  7  | 
  8  |     const emailid: string = "jagtap.swapnil@gmail.com";
  9  |     const password: string = "12345";
  10 | 
  11 |     await page.getByRole("textbox", { name: "email" }).fill(emailid);
  12 |     await page.getByRole("textbox", { name: "password" }).fill(password);
  13 |     await page.waitForTimeout(2000);
  14 |     await page.getByRole("checkbox", { name: "remember" }).check();
  15 |     await page.waitForTimeout(2000);
  16 |     await page.getByTestId("login-button").click();
  17 |     const pageurl:string = await page.url();
  18 |     console.log(pageurl);
> 19 |     await expect(page.url()).toContain(emailid);
     |                              ^ Error: expect(received).toContain(expected) // indexOf
  20 |     console.log("Email id validated");
  21 |     await expect(page.url()).toContain(password);
  22 |     console.log("Password validated");
