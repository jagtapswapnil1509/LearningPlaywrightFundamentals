...
- Name: Practice\practice5.spec.ts >> verify Dynamic webtable
- Location: tests\Practice\practice5.spec.ts:3:1

# Error details

```
Error: locator.innerText: Error: strict mode violation: locator('#employees-tbody tr td[data-col=\'email\']') resolved to 8 elements:
    1) <td data-col="email">mateo@tta.dev</td> aka getByRole('cell', { name: 'mateo@tta.dev' })
    2) <td data-col="email">camila@tta.dev</td> aka getByRole('cell', { name: 'camila@tta.dev' })
    3) <td data-col="email">diego@tta.dev</td> aka getByRole('cell', { name: 'diego@tta.dev' })
    4) <td data-col="email">valentina@tta.dev</td> aka getByRole('cell', { name: 'valentina@tta.dev' })
    5) <td data-col="email">lukas@tta.dev</td> aka getByRole('cell', { name: 'lukas@tta.dev' })
    6) <td data-col="email">hannah@tta.dev</td> aka getByRole('cell', { name: 'hannah@tta.dev' })
    7) <td data-col="email">felix@tta.dev</td> aka getByRole('cell', { name: 'felix@tta.dev' })
    8) <td data-col="email">mia@tta.dev</td> aka getByRole('cell', { name: 'mia@tta.dev' })

Call log:
  - waiting for locator('#employees-tbody tr td[data-col=\'email\']')
...
```ts
  ...
  2  | 
  3  | test('verify Dynamic webtable', async ({ page }) => {
  4  | 
  5  |     await page.goto("https://app.thetestingacademy.com/playwright/tables/webtable");
  6  | 
  7  |     // CSS = #employees-tbody tr
  8  |     // each value CSS = #employees-tbody tr td
  9  |     //const email = await page.locator("#employees-tbody tr td[data-col='email']").innerText();
  10 |     const eachtext = await page.locator("#employees-tbody tr td").allInnerTexts();
  11 |     //console.log(eachtext);
  12 |     let name:string = "Mia Hoffmann";
  13 |     let row;
  14 |     while(true){
  15 |         row = page.locator("#employees-tbody tr").filter({hasText:name});
  16 |         if(await row.count()){
> 17 |             const email = await page.locator("#employees-tbody tr td[data-col='email']").innerText();
     |                                                                                          ^ Error: locator.innerText: Error: strict mode violation: locator('#employees-tbody tr td[data-col=\'email\']') resolved to 8 elements:
  18 |             console.log("Email = ",email);
  19 |             break;
  20 |         }
  21 |         const next = await page.getByTestId("next-page");
  22 |         if(await next.isDisabled()) throw new Error("Row not found!");
  23 |         await next.click();
  24 |     }
  25 | });
```