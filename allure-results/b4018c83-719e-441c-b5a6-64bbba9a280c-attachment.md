...
Call log:
  - waiting for locator('//div[@class=\'QiMO5r\']/div').nth(73)
...
```ts
  ...
  11 |     const pricelocator:Locator = page.locator("//div[@class='hZ3P6w']");
  12 |     let countnumber:number=await pricelocator.count();
  13 |     for(let i=0;i<countnumber;i++){
> 14 |         const priceofeachitem:string|null = await page.locator("//div[@class='hZ3P6w']").nth(i).textContent();
     |                                                                                                     ^ Error: locator.textContent: Target page, context or browser has been closed
