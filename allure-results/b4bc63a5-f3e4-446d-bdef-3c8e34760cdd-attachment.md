...
- Name: 08_Web_Select_Frames_Iframe\260_Custom_dropdown.spec.ts >> Verify select from the custom dropdown
- Location: tests\08_Web_Select_Frames_Iframe\260_Custom_dropdown.spec.ts:3:1

# Error details

```
Error: locator.click: Error: strict mode violation: getByText('TypeScript', { exact: true }) resolved to 2 elements:
...
```ts
  ...
  8  |     //await page.locator("//div[@role='listbox']",{hasText:"TypeScript"}).click();
> 9  |     await page.getByText("TypeScript",{exact:true}).click();
     |                                                     ^ Error: locator.click: Error: strict mode violation: getByText('TypeScript', { exact: true }) resolved to 2 elements:
  10 |     await page.pause();
  11 | 
  12 | });
